...
def relu_forward(x):
    ...
    # If the value > 0 it will keep said value, else it will make it 0
    out = np.maximum(0, x)
...
def relu_backward(dout, cache):
    ...
    # Looks through all the x values and if greater than 0 it will keep the dout value
    dx = dout * (x > 0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    #nested loops go over: image, filter, height, width
    for i in range(N):  
        for f in range(F):  
...
def conv_backward_naive(dout, cache):
    ...
    N_dout, F_dout, Hp, Wp = dout.shape
    ...
    for i in range(N):  
        for f in range(F): 
            for h_out in range(Hp):
                for w_out in range(Wp):
                    h_start = h_out * stride
                    h_end = h_start + HH
                    w_start = w_out * stride
                    w_end = w_start + WW
        ...
        for c in range(C): 
            for hp in range(Hp): 
                for wp in range(Wp): 
                    ...
                    out[n, c, hp, wp] = np.max(pool_region)
    pass
...
def max_pool_backward_naive(dout, cache):
    ...
    N_dout, C_dout, Hp, Wp = dout.shape
        ...
        for c in range(C): 
            for hp in range(Hp):
                for wp in range(Wp): 
                    ...
                    dx[n, c, h_start:h_end, w_start:w_end] += mask * dout[n, c, hp, wp]
